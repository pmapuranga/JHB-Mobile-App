...
class BookProvider with ChangeNotifier {
  static const String _preloadDatabaseAsset = 'assets/preload/books.json';
  static const String _preloadPdfAssetPrefix = 'assets/preload/pdfs/';
  static const String _preloadVersionKey = 'preloaded_books_version';
  ...
  List<Book> getSortedBooks(BookSortType sortType, bool ascending) {
    ...
    switch (sortType) {
      ...
      case BookSortType.progress:
        sortedBooks
            .sort((a, b) => a.readingProgress.compareTo(b.readingProgress));
        break;
    }
  ...
  Future<void> loadBooks() async {
    ...
    try {
      ...
      await _importPreloadedBooks(prefs);
    } catch (e) {
  ...
  Future<void> saveBooks() async {
    try {
      final prefs = await SharedPreferences.getInstance();
      final booksJson =
          _books.map((book) => jsonEncode(book.toJson())).toList();
  ...
  Future<void> _importPreloadedBooks(SharedPreferences prefs) async {
    try {
      final databaseJson = await rootBundle.loadString(_preloadDatabaseAsset);
      final database = jsonDecode(databaseJson);
      final int version = database['version'] ?? 1;
      final List<dynamic> records = await _preloadRecords(database);

      if (records.isEmpty) {
        await prefs.setInt(_preloadVersionKey, version);
        return;
      }

      final appDir = await getApplicationDocumentsDirectory();
      final booksDir = Directory('${appDir.path}/preloaded_books');
      if (!await booksDir.exists()) {
        await booksDir.create(recursive: true);
      }

      final currentPreloadIds = records
      ...
      final beforePrune = _books.length;
      _books.removeWhere(
        (book) => book.isPreloaded && !currentPreloadIds.contains(book.id),
      );
      if (_books.length != beforePrune) {
        importedAny = true;
      }

      for (final record in records) {
        final book = await _bookFromPreloadRecord(record, booksDir);
        if (book == null) continue;

        final existingIndex =
            _books.indexWhere((existing) => existing.id == book.id);
        if (existingIndex == -1) {
          _books.add(book);
          importedAny = true;
        } else if (_books[existingIndex].isPreloaded ||
            currentPreloadIds.contains(_books[existingIndex].id)) {
      ...
      print('Error importing preloaded books: $e');
    }
  }

  Future<List<Map<String, dynamic>>> _preloadRecords(
    Map<String, dynamic> database,
  ) async {
    final configuredRecords = (database['books'] as List<dynamic>? ?? [])
        .whereType<Map<String, dynamic>>()
        .toList();

    if (configuredRecords.isNotEmpty) {
      return configuredRecords;
    }

    final manifestJson = await rootBundle.loadString('AssetManifest.json');
    final manifest = jsonDecode(manifestJson) as Map<String, dynamic>;
    final pdfAssets = manifest.keys
        .where((path) =>
            path.startsWith(_preloadPdfAssetPrefix) &&
            path.toLowerCase().endsWith('.pdf'))
        .toList()
      ..sort();

    return pdfAssets.map((assetPath) {
  ...
  Future<Book?> _bookFromPreloadRecord(
    Map<String, dynamic> record,
    Directory booksDir,
  ) async {
    final id = record['id']?.toString();
    final title = record['title']?.toString();
    final assetPath = record['asset']?.toString() ?? '';
    ...
    if (assetPath.isNotEmpty) {
      ...
      final fileName = _safeFileName(assetPath.split('/').last);
      copiedPath = '${booksDir.path}/$fileName';
      final copiedFile = File(copiedPath);

      if (!await copiedFile.exists()) {
        await copiedFile.writeAsBytes(assetBytes.buffer.asUint8List(),
            flush: true);
      }
    }
    ...
    return Book(
      id: 'preloaded:$id',
      idCode: record['idCode']?.toString() ?? '',
      title: title,
      filePath: copiedPath,
      addedDate: DateTime.tryParse(record['addedDate']?.toString() ?? '') ??
          DateTime(2026, 1, 1),
      location: record['location']?.toString() ?? 'Preloaded',
      duration: record['duration']?.toString() ?? 'TIME: 0:00',
      isPreloaded: true,
      sourceAssetPath: assetPath.isEmpty ? null : assetPath,
    );
  }
  ...
  Future<bool> addBook(String filePath, String fileName) async {
    try {
      ...
      // Пытаемся извлечь код из имени файла (например "47-0412 Faith Is The Substance.pdf")
      String idCode = '';
      String title = fileName.replaceAll('.pdf', '');

      final codeRegex = RegExp(r'^(\d{2}-\d{4})');
      final match = codeRegex.firstMatch(fileName);
      if (match != null) {
        idCode = match.group(1)!;
        title = title.replaceFirst(idCode, '').trim();
      }
      ...
      final book = Book(
        ...
        duration: 'TIME: 0:00', // Placeholder
      );
  ...
  int get completedBooks =>
      _books.where((book) => book.readingProgress >= 1.0).length;
  int get inProgressBooks => _books
      .where((book) => book.readingProgress > 0 && book.readingProgress < 1.0)
      .length;
  double get averageProgress {
    if (_books.isEmpty) return 0.0;
    final totalProgress =
        _books.fold<double>(0.0, (sum, book) => sum + book.readingProgress);
